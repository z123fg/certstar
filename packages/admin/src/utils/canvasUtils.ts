...
const ORIGINAL_HEIGHT = 3509;
const ZOOM = 0.4;

// ─── Default positions for each text field ───────────────────────────────────
const DEFAULT_TEXT_PROPS: Record<string, fabric.ITextOptions & { left: number; top: number }> = {
  name:         { left: 1175, top: 1765, fontSize: 60, fontFamily: "SimSun", fontWeight: 650, originX: "center", originY: "center", textAlign: "center" },
  idNum:        { left: 1175, top: 1942, fontSize: 60, fontFamily: "SimSun", fontWeight: 650, originX: "center", originY: "center", textAlign: "center" },
  organization: { left: 1175, top: 2100, fontSize: 60, fontFamily: "SimSun", fontWeight: 650, originX: "center", originY: "center", textAlign: "center" },
  certNum:      { left: 1175, top: 2265, fontSize: 60, fontFamily: "SimSun", fontWeight: 650, originX: "center", originY: "center", textAlign: "center" },
  expDate:      { left: 819,  top: 2435, fontSize: 60, fontFamily: "SimSun", fontWeight: 650, originX: "center", originY: "center", textAlign: "center" },
};

const DEFAULT_IMAGE_PROPS = {
  left: 1850, top: 1940,
  originX: "center", originY: "center",
  cornerStrokeColor: "#CD5C5C", cornerColor: "#A52A2A",
  borderColor: "#4169E1", transparentCorners: false,
...
// ─── Init / destroy ───────────────────────────────────────────────────────────
export const initCanvas = (canvasId = "main-canvas") => {
  destroyCanvas();
...
// ─── Template ────────────────────────────────────────────────────────────────
export const loadTemplate = (certType: string): Promise<void> =>
  new Promise((resolve) => {
    fabric.Image.fromURL(`/${certType}.jpg`, (img: fabric.Image) => {
      if (!canvas) return resolve();
      getObjsByEntry("template").forEach((o) => canvas!.remove(o));
      (img as FabricEntry).entry = "template";
      img.set({ selectable: false, lockMovementX: true, lockMovementY: true });
...
export const exportCanvasAsDataUrl = (): string =>
  canvas?.toDataURL({ format: "png", multiplier: 1 / ZOOM, quality: 1 }) ?? "";

export const downloadCanvasAsImage = async (filename = "certificate.png") => {
