...
export default function BatchPreviewPage({ token }: Props) {
    ...
    const { rows, setRows, rowLayouts, profileDataUrlOverrides, imageMap } =
        useBatchContext();
    ...
    const renderPhase = async (
        signal: AbortSignal,
        variant: CertVariant,
    ): Promise<{ rendered: Rendered[]; failed: string[] }> => {
        ...
        for (let i = 0; i < rows.length; i++) {
            ...
            try {
                ...
                rendered.push({
                    ...
                    certBlob,
                    profileDataUrl,
                    layout: savedLayout ?? getSnapshotLayout(),
                });
            } catch (err) {
    ...
    const handleSubmit = async () => {
        ...
        if (failed.length > 0) {
            setPageAlert({
                type: "error",
                message: `${failed.length} 条证书图片生成失败，请检查对应数据后重试：${failed.join("、")}`,
            });
            setStep("preview");
            return;
        }
        ...
        const payloads = new Array<
            Omit<Cert, "id" | "createdAt" | "updatedAt">
        >(rendered.length);
        try {
            await Promise.all(
                rendered.map(
                    async ({ draft, certBlob, profileDataUrl, layout }, i) => {
                        ...
                        if (profileDataUrl) {
                            const profileBlob = await fetch(profileDataUrl, {
                                signal,
                            }).then((r) => r.blob());
                        ...
                        setProgress((p) => {
                            const next = p + 1;
                            setProcessingMsg(
                                `正在上传图片 ${next - rows.length} / ${rendered.length}...`,
                            );
                            return next;
                        });
        ...
        setPageAlert({
            type: "success",
            message: `成功上传 ${payloads.length} 条证书`,
        });
        setStep("done");
    };
    ...
    const handleDownloadZip = async (variant: CertVariant) => {
        ...
        if (failed.length > 0) {
            setPageAlert({
                type: "error",
                message: `${failed.length} 条证书图片生成失败：${failed.join("、")}`,
            });
            setStep("preview");
            return;
        }
        ...
        const zip = new JSZip();
        rendered.forEach(({ draft, certBlob }) =>
            zip.file(`${draft.certNum}.pdf`, certBlob),
        );
        triggerBlobDownload(
            ...
            {pageAlert && (
                <Alert
                    ...
                    sx={{
                        mb: 2,
                        width: "100%",
                        maxWidth: step === "preview" ? "none" : 720,
                    }}
                >
                    {pageAlert.message}
                </Alert>
            ...
            {step === "preview" && (
                <Box sx={{ width: "100%" }}>
                    ...
                    >
                        ...
                        >
                            {intl.downloadZip}（{rows.length} 条）
                        </Button>
                        <Menu
                            ...
                            >
                                <span>
                                    <MenuItem
                                        ...
                                        onClick={() => handleDownloadZip("stamped")}
                                    >
                                        {intl.stamped}
                                    </MenuItem>
                                </span>
                            </Tooltip>
                            <MenuItem onClick={() => handleDownloadZip("stampless")}>
                                {intl.stampless}
                            </MenuItem>
                        </Menu>
                        <Tooltip title={complianceMode ? intl.complianceNoStampedSubmit : ""}>
                            <span>
                                <Button
                                    variant="contained"
                                    disabled={rows.length === 0 || complianceMode}
                                    onClick={handleSubmit}
                                >
                                    {intl.submitAll}（{rows.length} 条）
                                </Button>
                            </span>
                        </Tooltip>
                    </Stack>
                    ...
                    >
                        <Table size="small" stickyHeader>
                            ...
                            <TableBody>
                                {rows.map((row, i) => {
                                    const hasCustomLayout = rowLayouts.has(
                                        row._localId,
                                    );
                                    return (
                                            ...
                                            <TableCell>
                                                ...
                                                >
                                                    <IconButton
                                                        ...
                                                        onClick={() =>
                                                            navigate(
                                                                `/batch/draft/${encodeURIComponent(row._localId)}/edit`,
                                                            )
                                                        }
                                                    >
